...
class Thread {
  ...
  async _sendDMToUser(content, file = null) {
    ...
    if (! dmChannel) {
      throw new Error("Impossible d'ouvrir les messages directs avec l'utilisateur. Il a peut-être bloqué le bot ou a configuré des paramètres de confidentialité plus stricts.");
    }
  ...
  async _postToThreadChannel(content, file = null) {
    try {
      ...
      // Channel not found
      if (e.code === 10003) {
        console.log(`[INFO] Échec de l'envoi du message au canal du fil de discussion pour ${this.user_name} car le canal n'existe plus. Fermeture automatique du ticket.`);
        this.close(true);
      } else if (e.code === 240000) {
        console.log(`[INFO] Échec de l'envoi du message au canal du fil de discussion pour ${this.user_name} car le message contient un lien bloqué par le filtre de liens nuisibles.`);
        await bot.createMessage(this.channel_id, "Échec de l'envoi du message au canal du fil de discussion car le message contient un lien bloqué par le filtre de liens nuisibles.");
      } else {
        throw e;
      }
    }
  ...
  async replyToUser(moderator, text, replyAttachments = [], isAnonymous = false, messageReference = null) {
    ...
    if (! utils.messageContentIsWithinMaxLength(dmContent) || ! utils.messageContentIsWithinMaxLength(inboxContent)) {
      ...
      await this.postSystemMessage("La réponse est trop longue ! Assurez-vous que votre réponse ne dépasse pas 2000 caractères au total, y compris le nom du modérateur dans la réponse.");
      return false;
    }
    ...
    if (this.scheduled_close_at) {
      ...
      await this.postSystemMessage("Annulation de la fermeture prévue de ce ticket en raison d'une nouvelle réponse.");
    }
  ...
  async receiveUserReply(msg, skipAlert = false) {
    ...
    if (msg.activity) {
      ...
      messageContent += `\n\n*<Ce message contient une invitation à ${activityText} dans ${applicationName}>*`;
    ...
    if (this.scheduled_close_at) {
      await this.cancelScheduledClose();
      await this.postSystemMessage(`<@!${this.scheduled_close_id}> Le ticket qui devait être fermé a reçu une nouvelle réponse. Annulation.`, {
        allowedMentions: {
  ...
  async close(suppressSystemMessage = false, silent = false) {
    if (! suppressSystemMessage) {
      ...
      if (silent) {
        await this.postSystemMessage("Fermeture du ticket silencieusement...");
      } else {
  ...
  async editStaffReply(moderator, threadMessage, newText, opts = {}) {
    ...
    if (! utils.messageContentIsWithinMaxLength(formattedDM) || ! utils.messageContentIsWithinMaxLength(formattedThreadMessage)) {
      await this.postSystemMessage("La réponse modifiée est trop longue ! Assurez-vous que la modification ne dépasse pas 2000 caractères au total, y compris le nom du modérateur dans la réponse.");
      return false;
    }
  ...
  async recoverDowntimeMessages() {
    ...
    await this.postSystemMessage(`📥 Recupération ${messages.length} message(s) envoyés par l'utilisateur pendant le crash du bot !`);
