...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxServerCommand("snippet", "<trigger> [text$]", async (msg, args, thread) => {
    ...
    if (snippet) {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `L'extrait "${args.trigger}" existe déjà! Vous pouvez l'éditer ou le supprimer avec ${config.prefix}edit_snippet and ${config.prefix}delete_snippet respectively.`);
      } else {
      ...
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `L'extrait "${args.trigger}" a été crée!`);
      } else {
        // If the snippet doesn't exist and the user isn't trying to create it, inform them how to create it
        utils.postSystemMessageWithFallback(msg.channel, thread, `L'extrait "${args.trigger}" n'éxiste pas! Vous pouvez le créer avec \`${config.prefix}snippet ${args.trigger} text\``);
      }
    }
  }, {
    aliases: ["s"]
  ...
  commands.addInboxServerCommand("delete_snippet", "<trigger>", async (msg, args, thread) => {
    const snippet = await snippets.get(args.trigger);
    if (! snippet) {
      utils.postSystemMessageWithFallback(msg.channel, thread, `L'extrait "${args.trigger}" n'éxiste pas!`);
      return;
    }

    await snippets.del(args.trigger);
    utils.postSystemMessageWithFallback(msg.channel, thread, `L'extrait "${args.trigger}" a été supprimé!`);
  }, {
    aliases: ["ds"]
  });
  ...
  commands.addInboxServerCommand("edit_snippet", "<trigger> <text$>", async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `L'extrait "${args.trigger}" a été modifié!`);
  }, {
    aliases: ["es"]
  });
  ...
  commands.addInboxServerCommand("snippets", [], async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `Extraits disponibles (prefix ${config.snippetPrefix}):\n${triggers.join(", ")}`);
  }, {
    aliases: ["s"]
  });
};
