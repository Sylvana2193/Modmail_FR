...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function removeExpiredBlocks() {
    ...
    for (const userId of expiredBlocks) {
      ...
      logChannel.createMessage({
        content: `La sanction de <@!${userId}> (id \`${userId}\`) est terminée`,
        allowedMentions: {
          users: [userId],
        },
      });
  ...
  const blockCmd = async (msg, args, thread) => {
    ...
    if (isBlocked) {
      channel.createMessage("Le membre est déja bloqué");
      return;
    }
  ...
  const unblockCmd = async (msg, args, thread) => {
    ...
    if (! isBlocked) {
      msg.channel.createMessage("Le membre n'est pas bloqué");
      return;
    }
  ...
  commands.addInboxServerCommand("blocklist", "", async (msg, args, thread) => {
    ...
    if (blockedUsers.length === 0) {
      msg.channel.createMessage("Pas d'utilisateur actuellement bloqué.");
      return;
    }

    let reply = "List of blocked users:\n";
    for (const user of blockedUsers) {
      const userInfo = `**<@!${user.userId}> (id \`${user.userId}\`)** - bloque par <@${user.blockedBy}>${user.expiresAt ? ` jusqu'a ${user.expiresAt} (UTC)` : " permanently"}`;
      reply += userInfo + "\n";
    }
