...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand("role", [], async (msg, args, thread) => {
    ...
    if (displayRole) {
      thread.postSystemMessage(`Votre rôle d'affichage dans cette conversation est actuellement **${displayRole}**`);
    } else {
      thread.postSystemMessage("Vos réponses dans cette conversation n'affichent actuellement aucun rôle.");
    }
  }, { allowSuspended: true });
  ...
  commands.addInboxThreadCommand("role reset", [], async (msg, args, thread) => {
    ...
    const displayRole = await getModeratorThreadDisplayRoleName(msg.member, thread.id);
    if (displayRole) {
      thread.postSystemMessage(`Votre rôle d'affichage pour cette conversation a été réinitialisé. Vos réponses afficheront désormais le rôle par défaut **${displayRole}**.`);
    } else {
      thread.postSystemMessage("Votre rôle d'affichage pour cette conversation a été réinitialisé. Vos réponses n'afficheront plus de rôle.");
    }
  }, {
  ...
  commands.addInboxThreadCommand("role", "<role:string$>", async (msg, args, thread) => {
    ...
    if (! role || ! msg.member.roles.includes(role.id)) {
      thread.postSystemMessage("Aucun rôle correspondant trouvé. Assurez-vous de disposer du rôle avant d'essayer de le définir comme votre rôle d'affichage dans cette conversation.");
      return;
    }

    await setModeratorThreadRoleOverride(msg.member.id, thread.id, role.id);
    thread.postSystemMessage(`Votre rôle d'affichage pour cette conversation a été défini sur **${role.name}**. Vous pouvez le réinitialiser avec \`${config.prefix}role reset\`.`);
  }, { allowSuspended: true });
  ...
  commands.addInboxServerCommand("role reset", [], async (msg, args, thread) => {
    ...
    const displayRole = await getModeratorDefaultDisplayRoleName(msg.member);
    if (displayRole) {
      channel.createMessage(`Votre rôle d'affichage par défaut a été réinitialisé. Vos réponses afficheront désormais le rôle **${displayRole}** par default.`);
    } else {
  ...
  commands.addInboxServerCommand("role", "<role:string$>", async (msg, args, thread) => {
    ...
    const role = resolveRoleInput(args.role);
    if (! role || ! msg.member.roles.includes(role.id)) {
      channel.createMessage("Aucun rôle correspondant trouvé. Assurez-vous de disposer du rôle avant d'essayer de le définir comme votre rôle d'affichage par défaut.");
      return;
    }
