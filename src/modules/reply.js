...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  if (config.allowStaffEdit) {
    commands.addInboxThreadCommand("edit", "<messageNumber:number> <text:string$>", async (msg, args, thread) => {
      ...
      if (! threadMessage) {
        utils.postError(msg.channel, "Le numéro du message indiqué n'est pas reconnu");
        return;
      }

      if (threadMessage.user_id !== msg.author.id) {
        utils.postError(msg.channel, "Vous pouvez modifier uniquement votre propre message");
        return;
      }
  ...
  if (config.allowStaffDelete) {
    commands.addInboxThreadCommand("delete", "<messageNumber:number>", async (msg, args, thread) => {
      ...
      if (threadMessage.user_id !== msg.author.id) {
        utils.postError(msg.channel, "Vous ne pouvez supprimer que vos propres messages");
        return;
      }
