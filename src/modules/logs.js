...
module.exports = ({ bot, knex, config, commands, hooks }) => {
  ...
  const logsCmd = async (msg, args, thread) => {
    ...
    let message = isPaginated
      ? `**Les logs de <@${userId}>** (page **${page}/${maxPage}**, vue sur les logs **${start + 1}-${end}/${totalUserThreads}**):`
      : `**Les logs de <@${userId}>:**`;

    message += `\n${threadLines.join("\n")}`;

    if (isPaginated) {
      message += "\nPour voir plus, ajouter un numéro de page à la fin de la commande";
    }

    if (threadLines.length === 0) message = `**Il n y a pas de log pour <@${userId}>**`;
  ...
  const logCmd = async (msg, args, _thread) => {
    ...
    if (logUrl) {
      channel.createMessage(`Ouvrez ce lien pour voir le log du ticket #${thread.thread_number}:\n<${addOptQueryStringToUrl(logUrl, args)}>`);
      return;
    }

    const logFile = await getLogFile(thread);
    if (logFile) {
      channel.createMessage(`Téléchargez ce fichier pour voir le log du ticket #${thread.thread_number}:`, logFile);
      return;
    }

    if (thread.status === THREAD_STATUS.OPEN) {
      channel.createMessage(`Les logs de ce ticket ne sont pas disponible pour le moment, mais sont ouverts sur <#${thread.channel_id}>`);
      return;
    }

    channel.createMessage("Les logs de ce ticket ne sont pas disponible pour le moment");
  };
