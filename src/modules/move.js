...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand("move", "<category:string$>", async (msg, args, thread) => {
    ...
    if (containsRankings[0][1] === 0) {
      thread.postSystemMessage("Pas de category correspondant");
      return;
    }
    ...
    try {
      ...
      thread.postSystemMessage(`Erreur de deplacement du ticket: ${e.message}`);
      return;
    }
    ...
    if (config.syncPermissionsOnMove) {
      ...
      try {
        ...
        thread.postSystemMessage(`Le ticket a été déplacer vers **${targetCategory.name.toUpperCase()}**, mais a rencontré des problemes de syncronisation avec les permissions de la categorie de destination: ${e.message}`);
        return;
      }
    }

    thread.postSystemMessage(`Le ticket a été déplacé vers **${targetCategory.name.toUpperCase()}**`);
  });
};
